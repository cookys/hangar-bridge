...
## 現況
- 分支 `feat/reply-routing`（從 develop `39c7cc5` 開）。已 merge：**D1**（`f88e9e5`，shared）、**D2**（`934cd62`，relay data layer）、**D3**（`dd27712`，impl/reply-routing-D3 = `ef18c3e`，9 commits：addressRules 旗標 default off、return selector、snapshot→交易→deliver 寫入順序、§7 thread 續接、§6 四種旗標拒絕、audience report、stream/poll grants）。D3 merge 前獨立 gate 綠（relay 334 tests / lines 93.6 %）。**D4 hands 進行中**（`impl/reply-routing-D4`，基底 dd27712，prompt `/tmp/autopilot-campaigns/D4.prompt.md`，reviewer spec `/tmp/qc/D4.spec.md`）。
