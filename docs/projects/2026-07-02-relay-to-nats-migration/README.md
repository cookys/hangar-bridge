...
> `2c66f10` 已納入主線；P5/P6 沒有 feature branch 在執行。
...
| Docs | closeout 已補雙 transport 現況與 claims compatibility；Phase 6 仍需把退役 SSE/relay 章節改成 historical note |
...
- 2026-07-21:**mainline closeout**。P0–P4 與 fleet-coordination stage3（subject-scoped
  `@team` chat、connection-backed presence、SQLite v6 claims）完成整合。SSE 保持 default，NATS
  保持 opt-in。NATS 訊息啟動不依賴 relay，但 claims 在 P5 仍走 optional relay compatibility
  client；P6 前必須 port 或明確 retire claims。
- 2026-07-21 closeout hardening：durable ACK 等待 async MCP delivery + KV completion marker；KV
  未就緒時 task consumer 關閉並 retry；live 測試改用正式最小權限等價 ACL；NATS 同 handle
  暫限單一 local session。結構化 `task_result` MCP response tool 明確 deferred，現況以 chat
  回報 completion。Correlation state 已改為 atomic disk-backed JSON，原 plan 的 restart-loss
  敘述由本 closeout 決策取代。
- 2026-07-02:**P3 完成(`64565d0`)**。hetero implementer(gpt-5.3-codex-spark)**撞 API 用量上限**(限流至 21:26),依 /l6 文件化 `--solo` 降級:CEO(Claude)depth-0 自己實作,獨立 Claude harness(盲寫)驗證 —— 本階 cross-family 去相關不可得(誠實記錄),但獨立 harness + 兩套 live 證明仍把關。KV atomic-create 去重,重複 signature live 驗證為 `JetStreamApiError code 10071`。獨立 harness 3 fail 全為 ABI 猜測(async `openTaskDedup(nc,selfHandle,{kvm})` vs 猜的同步 ctor、`seen():boolean` vs 猜的字串)—— reconcile harness glue 而非 impl。該 commit 當時採 bounded background KV-open；2026-07-21 closeout 已將它取代為「KV ready 前不開 durable consumer + retry backoff」。205 tests、task-dedup 100%。
