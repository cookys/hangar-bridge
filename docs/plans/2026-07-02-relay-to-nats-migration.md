...
> **Status:** 🟡 IN LOOP-REVIEW — g55xh (gpt-5.5 xhigh) blocking trajectory 7→5→5→6→… (R1–R4 fixed).
> **Owner:** cookys (Board)
...
- **AC12 — Subscribe chokepoint preserved (only the addressed recipient receives).** Each user's `subscribe.allow` is exactly `["fleet.*.to.<handle>.>", "fleet.*.to.team.>"]`, deny-by-default (§2.6). This reproduces the relay subscribe chokepoint (architecture §5.5 / SUBJECT_ROUTING_SPEC §4.3): a peer cannot subscribe to another peer's `to.<other>` traffic. Gate: (a) `nats-config.test.ts` asserts each user's `subscribe.allow` matches the recipient-scoped pattern and nothing broader (no `fleet.>`, no bare `_INBOX.>`); (b) a negative integration test proves a peer subscribing to `fleet.*.to.<other-handle>.>` is DENIED / receives nothing; (c) a self-delivery test proves a broadcaster does NOT receive its own `to.team` message (§2.6 self-drop); (d) `nats-config.test.ts` asserts the FLEET roster handles (`fleet-roster.json`) and the conf FLEET NKey users are the EXACT same set (no orphan fleet user, no roster-only handle) — the `$SYS`/admin credential is excluded from both the roster and this equality check; (e) a test asserts no fleet handle equals the reserved broadcast token `team` (§2.6).
...
- **Subscribe ACL (subscribe chokepoint, AC12):** each user `subscribe.allow = ["fleet.*.to.<handle>.>", "fleet.*.to.team.>"]` — a peer receives ONLY messages addressed to its own handle or to the team lane; it CANNOT subscribe to another peer's `to.<other>` traffic. Deny-by-default otherwise. **Self-delivery drop:** because a broadcaster also matches its own `fleet.*.to.team.>`, the peer-agent applies an app-side receive gate that DROPS any message where `recipient == team && sender == localHandle` — reproducing the relay's "fan out to every online handle EXCEPT `from`" (architecture §5.5).
