...
> **Status:** 🟡 IN LOOP-REVIEW — g55xh (gpt-5.5 xhigh) blocking trajectory 7→5→5→… (R1–R3 fixed).
> **Owner:** cookys (Board)
...
- **Publish ACL (anti-spoof, AC2/AC2b):** each user `publish.allow = ["fleet.<handle>.>"]` — a peer may publish ONLY under its own sender handle, so the SECOND subject token is a NATS-enforced true sender identity. The receiving peer-agent derives `from` from that token, NEVER from the envelope `from` field.
...
- **Broadcast (`to.team`) lane is core-NATS `chat`/`presence_update` ONLY.** `task_dispatch` is ALWAYS a per-handle direct DM, NEVER `@team` (SUBJECT_ROUTING_SPEC R1 — a subjected/`task_dispatch` `@team` was a hard 400 on the relay). The peer-agent HARD-REJECTS `kind == task_dispatch && to == @team` (regardless of `subject`); a hub dispatching to a namespace fans out ONE direct `fleet.<sender>.to.<owner>.task_dispatch` per roster owner. This keeps the JetStream `WorkQueuePolicy` stream (which is single-consumer, not broadcast) carrying only direct task messages, never a `to.team` task.
- **Wire-subject grammar gate (`parseFleetSubject`, AC13):** every inbound wire subject MUST parse as exactly the 5 tokens `fleet.<sender>.to.<recipient>.<kind>` with `<kind>` one of the six envelope kinds. The peer-agent REJECTS the message if: the subject is malformed / has extra tokens; the `<sender>` token ≠ the NATS-authenticated publish principal; the `<recipient>` token ≠ the local handle (or `team` for a broadcast the peer legitimately subscribes to); or `<kind>` ≠ the JSON `Envelope.kind`. This makes the wire subject — not the forgeable envelope body — the identity/routing authority.
...
- **Roster + namespace-ownership SoT (replaces `peers.json`/`human.subjects`):** the app-side envelope-subject ACL (AC11) needs an authority for who owns which namespace, which the retired relay read from `human.subjects`. This plan ships that authority as a new **`packages/operations/nats/fleet-roster.json`** (per-handle `owned`/`interest` namespaces + display name), distributed to every host and consumed by the peer-agent for `list_peers`, the inbound sender gate, and AC11's publisher/recipient ownership checks. It is the single membership + ownership SoT after `peers-file.ts` retires; a relay restart's re-seed discipline is replaced by re-reading the roster on peer-agent (re)start. **Roster⇔NATS-users equality (AC12 gate):** the roster handles and the `nats-server.conf` NKey users MUST be the exact same set — `nats-config.test.ts` fails on any orphan NKey user (in conf but not roster) or roster-only handle (in roster but no NKey user). `list_peers` returns the roster handles annotated with heartbeat state, NOT a presence-discovered membership.
