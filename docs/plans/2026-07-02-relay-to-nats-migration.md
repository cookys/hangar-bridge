...
> **Status:** 🟡 IN LOOP-REVIEW — g55xh (gpt-5.5 xhigh) blocking trajectory 7→5→… (R1, R2 fixed).
> **Owner:** cookys (Board)
...
- **Subscribe ACL (subscribe chokepoint, AC12):** each user `subscribe.allow = ["fleet.*.to.<handle>.>", "fleet.*.to.team.>"]` — a peer receives ONLY messages addressed to its own handle or to the team lane; it CANNOT subscribe to another peer's `to.<other>` traffic. Deny-by-default otherwise.
