 ...
 *
 * Robustness contract:
 *   - Completed Phase-0 artifacts are mandatory; absence fails suite setup.
...
const GENKEYS_PATH = path.join(NATS_DIR, 'genkeys.sh')
...
const genkeysPresent = fs.existsSync(GENKEYS_PATH)
 ...
 * prefix and permits at most a single trailing `.>`/`.*`. KV reads are direct; no
 * KV-watch consumer-management grant belongs on a fleet identity.
 */
const CONTROL_PUB_ALLOWLIST: RegExp[] = [
  /^\$JS\.ACK\.[^>*]+(\.[>*])?$/, // ack for own stream/consumer
  /^\$JS\.API\.CONSUMER\.MSG\.NEXT\.[^>*]+(\.[>*])?$/, // pull next for own task consumer
  /^\$JS\.API\.CONSUMER\.INFO\.[^>*]+(\.[>*])?$/, // own task-consumer info
...
// ===========================================================================
// ARTIFACT SCANS — completed Phase-0 files are mandatory
// ===========================================================================
let CONF_RAW = ''
...
beforeAll(() => {
  ...
  if (!genkeysPresent) missing.push(`genkeys.sh (${GENKEYS_PATH})`)
  if (missing.length) {
    throw new Error(`completed Phase-0 artifact(s) missing:\n  - ${missing.join('\n  - ')}`)
  }
  CONF_RAW = fs.readFileSync(CONF_PATH, 'utf8')
  CONF = extractConf(parseNatsConf(CONF_RAW, NATS_DIR))
  ROSTER_JSON = JSON.parse(fs.readFileSync(ROSTER_PATH, 'utf8'))
  PROVISION_RAW = fs.readFileSync(PROVISION_PATH, 'utf8')
  if (!serverAvail) {
...
describe('NATS operations scripts — POSIX shell syntax', () => {
  it.skipIf(!provisionPresent || !genkeysPresent)('parses provisioning and key-generation scripts with sh -n', () => {
    const result = spawnSync('sh', ['-n', PROVISION_PATH, GENKEYS_PATH], { encoding: 'utf8' })
    expect(result.status, result.stderr || result.error?.message).toBe(0)
  })

  it.skipIf(!genkeysPresent)('rejects invalid/reserved handles before generating or exposing a seed', () => {
    for (const handle of ['bad/handle', '../alpha', 'UPPER', 'team', 'a'.repeat(33)]) {
      const result = spawnSync('sh', [GENKEYS_PATH, handle], { encoding: 'utf8' })
      expect(result.status, `handle=${handle}`).not.toBe(0)
      expect(result.stdout, `handle=${handle}`).not.toContain('seed=')
      expect(result.stdout, `handle=${handle}`).not.toContain('path=')
    }
  })

  it('reconciles a drifted durable consumer and preserves an exact one', () => {
    const tmp = fs.mkdtempSync(path.join(process.env.TMPDIR || '/tmp', 'nats-provision-fake-'))
    const fakeNats = path.join(tmp, 'nats')
    const logPath = path.join(tmp, 'calls.log')
    const rosterPath = path.join(tmp, 'roster.json')
    const seedPath = path.join(tmp, 'admin.nk')
    fs.writeFileSync(rosterPath, JSON.stringify({ alice: {}, bob: {} }))
    fs.writeFileSync(seedPath, 'SUFAKEADMINSEED\n', { mode: 0o600 })
    fs.writeFileSync(fakeNats, `#!/bin/sh
set -eu
printf '%s\\n' "$*" >> "$FAKE_NATS_LOG"
case " $* " in
  *" stream info HANGAR_TASKS --json "*)
    printf '%s\n' '{"config":{"retention":"workqueue","storage":"file","num_replicas":1}}'
    ;;
  *" stream info KV_HANGAR_DEDUP --json "*)
    printf '%s\n' '{"config":{"retention":"limits","storage":"file","num_replicas":1,"max_msgs_per_subject":1,"max_age":0,"allow_direct":true}}'
    ;;
  *" consumer list HANGAR_TASKS --names --json "*)
    printf '%s\n' '["alice","bob","retired"]'
    ;;
  *" consumer info HANGAR_TASKS alice --json "*)
    printf '%s\\n' '{"config":{"filter_subject":"fleet.*.to.wrong.>","ack_policy":"none","deliver_subject":"push.inbox"}}'
    ;;
  *" consumer info HANGAR_TASKS bob --json "*)
    printf '%s\\n' '{"config":{"filter_subject":"fleet.*.to.bob.>","ack_policy":"explicit"}}'
    ;;
esac
`, { mode: 0o755 })

    try {
      const result = spawnSync('sh', [PROVISION_PATH], {
        encoding: 'utf8',
        env: {
          ...process.env,
          NATS_BIN: fakeNats,
          NATS_URL: 'nats://fake.invalid:4222',
          NATS_ADMIN_SEED_PATH: seedPath,
          ROSTER_FILE: rosterPath,
          FAKE_NATS_LOG: logPath,
        },
      })
      expect(result.status, result.stderr || result.error?.message).toBe(0)
      const calls = fs.readFileSync(logPath, 'utf8')
      expect(calls).toContain('consumer rm HANGAR_TASKS alice -f')
      expect(calls).toMatch(/consumer add HANGAR_TASKS alice --filter fleet\.\*\.to\.alice\.> --pull --ack explicit --defaults/)
      expect(calls).not.toContain('consumer rm HANGAR_TASKS bob -f')
      expect(calls).not.toMatch(/consumer add HANGAR_TASKS bob /)
      expect(calls).toContain('consumer rm HANGAR_TASKS retired -f')
    } finally {
      fs.rmSync(tmp, { recursive: true, force: true })
    }
  })

  it('fails closed when an existing task stream has unsafe durability drift', () => {
    const tmp = fs.mkdtempSync(path.join(process.env.TMPDIR || '/tmp', 'nats-provision-drift-'))
    const fakeNats = path.join(tmp, 'nats')
    const rosterPath = path.join(tmp, 'roster.json')
    const seedPath = path.join(tmp, 'admin.nk')
    fs.writeFileSync(rosterPath, JSON.stringify({ alice: {} }))
    fs.writeFileSync(seedPath, 'SUFAKEADMINSEED\n', { mode: 0o600 })
    fs.writeFileSync(fakeNats, `#!/bin/sh
set -eu
case " $* " in
  *" stream info HANGAR_TASKS --json "*)
    printf '%s\n' '{"config":{"retention":"limits","storage":"memory","num_replicas":3}}'
    ;;
esac
`, { mode: 0o755 })
    ...
    try {
      ...
      expect(result.stderr).toMatch(/unsafe immutable drift/)
    } finally {
      fs.rmSync(tmp, { recursive: true, force: true })
    }
  })
