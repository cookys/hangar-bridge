 ...
 * Only an explicitly supplied mutation-test module may be absent and skip.
 * The default completed P2 module and its dependencies MUST load or fail setup.
 *
...
const MUTATION_OVERRIDE = process.env.P2_MODULE_SPECIFIER !== undefined
...
interface TransportOptsLike {
  ...
  onEnvelope: (env: Envelope) => 'delivered'
  onAuthError: () => void
...
async function loadTransportCtor(): Promise<TransportCtor | null> {
  try {
    ...
    if (m.NatsTransport) return m.NatsTransport
    if (MUTATION_OVERRIDE) return null
    throw new Error(`completed P2 module has no NatsTransport export: ${MOD_SPEC}`)
  } catch (error) {
    if (MUTATION_OVERRIDE) return null
    throw error
  }
...
function makeTestDedup() {
  const completed = new Set<string>()
  return {
    seen: async (key: string) => completed.has(key),
    classify: async (key: string) => completed.has(key) ? 'duplicate' : 'new',
    isCompleted: async (key: string) => completed.has(key),
    markCompleted: async (key: string) => { completed.add(key) },
  }
}
...
async function startWithFakes(selfHandle: string) {
  ...
  const t = new Ctor({
    ...
    roster: ROSTER,
    onEnvelope: env => { received.push(env); return 'delivered' },
    onAuthError: () => {},
  ...
  await t.start()
  // Startup emits an immediate presence heartbeat. This matrix measures only
  // the explicit send() under test, so exclude that lifecycle publish.
  fakeNc.publishCalls.length = 0
  fakeJs.publishCalls.length = 0
...
describe('P2 AC6 · LIVE two-tier durability (JetStream replay vs core loss)', () => {
  it(
    ...
    async ctx => {
      ...
      // ── infra bring-up: once the optional server binary exists, failures FAIL ──
      try {
