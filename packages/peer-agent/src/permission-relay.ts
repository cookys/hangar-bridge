...
export interface OutboundPermissionRelayDeps {
   ...
   */
  outboundTracker?: Pick<PermissionOutboundTracker, 'recordRelay' | 'confirm' | 'revoke'> | undefined
  now?: () => Date
}
...
export function makeOutboundPermissionHandler(deps: OutboundPermissionRelayDeps) {
  ...
  return async function relay(params: OutboundPermissionRequestParams): Promise<{ relayedTo: string[] }> {
    ...
    for (const to of targets) {
      try {
        const envelope = await deps.client.send(
          buildOutboundPermissionRequest(params, to, deps.selfHandle, expiresAt),
        )
        deps.outboundTracker?.confirm(params.request_id, to, envelope.id)
        relayed.push(to)
      } catch (err) {
