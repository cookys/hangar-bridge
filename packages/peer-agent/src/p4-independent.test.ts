 ...
 * (tsc stays clean; the module surface is `any`). Default completed modules
 * must load; only explicit mutation overrides may be absent and skip.
 *
...
const TRACKER_OVERRIDE = process.env.P4_PRESENCE_MODULE !== undefined
const TRANSPORT_OVERRIDE = process.env.P4_TRANSPORT_MODULE !== undefined
...
if (TRACKER_ABSENT && !TRACKER_OVERRIDE) {
  throw new Error(`completed P4 presence tracker export missing: ${TRACKER_SPEC}`)
}
...
try {
  ...
  if (!TRANSPORT_OVERRIDE) throw e
  transportLoadError = e
}
const TRANSPORT_ABSENT = typeof NatsTransport !== 'function'
if (TRANSPORT_ABSENT && !TRANSPORT_OVERRIDE) {
  throw new Error(`completed P4 transport export missing: ${TRANSPORT_SPEC}`)
}
...
function newTransport(selfHandle: string, conn: any, now: () => number) {
  return new NatsTransport({
    ...
    roster,
    onEnvelope: () => 'delivered' as const,
    onAuthError: () => {},
    instanceGuard: { acquire: () => {}, release: () => {} },
    auditWriter: () => {},
...
describe('AC7 — transport integration: heartbeat drives listPeers (clause 5, in-memory bus)', () => {
  it.skipIf(TRANSPORT_ABSENT)('an inbound presence_update onlines the sender with a non-null last_seen; TTL expiry offlines it; a never-heartbeat peer is offline', async () => {
    ...
    try {
      ...
      // Before beta starts (and before its immediate startup heartbeat): offline.
      ...
      // Starting beta emits an immediate real presence_update heartbeat.
      const hbTime = clock.t
      await beta.start()
      const betaStarted = await pollPeer(alpha, 'beta', (p) => p.online === true)
      expect(betaStarted?.online).toBe(true)
      expect(betaStarted?.last_seen).toBe(new Date(hbTime).toISOString())

      // Manual presence updates use the same heartbeat path.
