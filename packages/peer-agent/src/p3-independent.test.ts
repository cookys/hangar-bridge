...
const DEDUP_OVERRIDE = process.env.P3_DEDUP_MODULE !== undefined
...
async function tryImport(spec: string, optionalMutation = false): Promise<Record<string, any> | null> {
  try {
    return (await import(/* @vite-ignore */ spec)) as Record<string, any>
  } catch (error) {
    if (optionalMutation) return null
    throw error
  }
}

// Loaded at module top-level so it.skipIf() (evaluated at collection) can see it.
const dedupMod = await tryImport(DEDUP_SPEC, DEDUP_OVERRIDE)
...
if (typeof NatsTransport !== 'function') {
  throw new Error(`completed P3 transport export missing: ${TRANSPORT_SPEC}`)
}
...
describe('P3 KV-dedup — transport consume-path integration', () => {
  it.skipIf(!TRANSPORT_PRESENT)(
    ...
    async () => {
      ...
      seam.classify = async (arg: any, envelopeId: string) => {
        const key = typeof arg === 'string' ? arg : (arg?.id ?? JSON.stringify(arg))
        const existing = wireIds.get(key)
        if (existing === undefined) {
          wireIds.set(key, envelopeId)
          return 'new'
        }
        return existing === envelopeId ? 'retry' : 'duplicate'
      }
      seam.isCompleted = async (arg: any) => {
        const key = typeof arg === 'string' ? arg : (arg?.id ?? JSON.stringify(arg))
        return seen.has(key)
      }
      seam.markCompleted = async (arg: any) => {
        const key = typeof arg === 'string' ? arg : (arg?.id ?? JSON.stringify(arg))
        seen.add(key)
      }
      ...
      const t = new NatsTransport({
        ...
        roster: {
          [SELF]: { owned: [], interest: [] },
          openclaw: { owned: [], interest: [] },
        },
        onEnvelope: (e: any) => { rec.onEnv.push(e); return 'delivered' as const },
        onAuthError: () => {},
