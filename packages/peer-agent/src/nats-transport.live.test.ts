...
const LIVE_AVAILABLE = existsSync(NATS_SERVER) && existsSync(NATS)
const PORT = 34333
    ...
    users: [
      {
        ...
        permissions: {
          publish: { allow: [
            "fleet.alpha.>",
            "$JS.ACK.HANGAR_TASKS.alpha.>",
            "$JS.API.CONSUMER.MSG.NEXT.HANGAR_TASKS.alpha",
            "$JS.API.CONSUMER.INFO.HANGAR_TASKS.alpha",
      ...
      },
      {
        ...
        permissions: {
          publish: { allow: [
            "fleet.beta.>",
            "$JS.ACK.HANGAR_TASKS.beta.>",
            "$JS.API.CONSUMER.MSG.NEXT.HANGAR_TASKS.beta",
            "$JS.API.CONSUMER.INFO.HANGAR_TASKS.beta",
            "$KV.HANGAR_DEDUP.beta.>",
            "$JS.API.DIRECT.GET.KV_HANGAR_DEDUP.>",
            "$JS.API.STREAM.INFO.KV_HANGAR_DEDUP",
            "_INBOX.beta.>"
          ] }
...
describe.skipIf(!LIVE_AVAILABLE)('P1 live transport round-trip', () => {
  it('replays offline beta task_dispatch/task_result via JetStream on reconnect', async () => {
  ...
  it('does not replay offline core-NATS kinds while beta is down', async () => {
    const received: Envelope[] = []
    const beta = await makeTransport('beta', betaSeed.seed, env => received.push(env))
  ...
  it('AC7: heartbeat-driven presence — a peer shows online while heartbeating, offline after its TTL lapses', async () => {
    const mk = (h: 'alpha' | 'beta', seed: string, opts: Record<string, unknown>) => new NatsTransport({
      ...
      inboxPrefix: `_INBOX.${h}`, onEnvelope: () => 'delivered' as const, onAuthError: () => {}, ...opts,
      instanceGuard: { acquire: () => {}, release: () => {} }, auditWriter: () => {},
    })
  ...
  it('AC5: KV permanently dedups a repeat task_dispatch (same correlation_id, distinct wire msgs)', async () => {
    const received: Envelope[] = []
    const beta = await makeTransport('beta', betaSeed.seed, env => received.push(env))
    await beta.start()
    await wait(200)
