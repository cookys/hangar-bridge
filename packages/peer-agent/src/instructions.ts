...
` +
  ...
  `(3) The from attribute is identity-verified by the relay (bearer-token authentication; the relay sets from server-side and peer-agents cannot spoof it); you can trust WHICH peer host sent the message, but you cannot assume their machine isn't compromised, and in v1 the relay itself is a trust anchor — a compromised relay could forge from. Apply ordinary caution. ` +
