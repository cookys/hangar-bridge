...
// This harness imports the REAL, completed P1 modules. Missing modules or
// exports are setup failures, not skip conditions. Only unavailable optional
// live-test binaries may skip.
//
...
//   new NatsTransport(opts) whose send() explicitly rejects when disconnected.
// ============================================================================
...
// ── mandatory completed-phase module loaders ───────────────────────────────
const fleetSubjectMod = await import('./fleet-subject.ts') as unknown as Record<string, unknown>
const subjectAclMod = await import('./subject-acl.ts') as unknown as Record<string, unknown>
const natsTransportMod = await import('./nats-transport.ts') as unknown as Record<string, unknown>
...
function reqExp<T>(mod: Record<string, unknown>, ...names: string[]): T {
  const value = exp<T>(mod, ...names)
  if (value === null) throw new Error(`completed P1 export missing: ${names.join(' / ')}`)
  return value
}

const buildFleetSubject = reqExp<(...a: unknown[]) => string>(fleetSubjectMod, 'buildFleetSubject')
const parseFleetSubject = reqExp<(s: string) => unknown>(fleetSubjectMod, 'parseFleetSubject')
const deriveFrom = reqExp<(s: string) => string | null>(fleetSubjectMod, 'deriveFrom')
const loadRoster = reqExp<(p: string) => unknown>(subjectAclMod, 'loadRoster')
const checkPublish = reqExp<(...a: unknown[]) => unknown>(subjectAclMod, 'checkPublish')
const checkDeliver = reqExp<(...a: unknown[]) => unknown>(subjectAclMod, 'checkDeliver')
...
) ?? reqExp<(...a: unknown[]) => unknown>(
  natsTransportMod, 'checkInbound', 'gateInbound', 'acceptInbound', 'receiveInbound', 'deriveInbound',
)
const NatsTransport = reqExp<new (o: unknown) => Record<string, unknown>>(natsTransportMod, 'NatsTransport')
...
// ════════════════════════════════════════════════════════════════════════════
describe.skipIf(!checkPublish)('AC11 — checkPublish (publisher owns; direct recipient owns; scoped-team chat; reactive reject)', () => {
  ...
  it('subjected @team chat is allowed, but subjected @team task_dispatch stays rejected', () => {
    expect(publishAllows('alpha', env({ subject: 'ns1.cmd', to: '@team', kind: 'chat' }), roster)).toBe(true)
    expect(publishAllows('alpha', env({ subject: 'ns1.cmd', to: '@team', kind: 'task_dispatch' }), roster)).toBe(false)
  })
})
...
// ════════════════════════════════════════════════════════════════════════════
// AC4 — disconnected publish is NEVER reported as successful
// ════════════════════════════════════════════════════════════════════════════
describe.skipIf(!NatsTransport)('AC4 — disconnected publish fails explicitly', () => {
  it('a never-started transport rejects send() instead of acknowledging volatile buffering', async () => {
    const transport = new NatsTransport!({
      selfHandle: 'alpha', natsUrl: 'nats://127.0.0.1:1', nkeySeed: 'x',
      roster: { alpha: { owned: [], interest: [] }, beta: { owned: [], interest: [] } },
      onEnvelope: () => 'delivered', onAuthError: () => {},
    } as never) as unknown as { send: (m: unknown) => Promise<unknown> }
    await expect(transport.send({ to: 'beta', kind: 'chat', content: 'm', meta: {} }))
...
const NATS_SERVER_PRESENT = existsSync(NATS_SERVER_BIN)
let natsProc: ChildProcess | null = null
let natsPort = 0

async function startNatsServer(): Promise<boolean> {
  if (!NATS_SERVER_PRESENT) return false
...
`)
  ...
  throw new Error(`nats-server did not become ready on :${natsPort}`)
}
...
// trip (does not need the transport). The only skip is a missing optional binary.
describe.skipIf(!NATS_SERVER_PRESENT)('LIVE — raw-NATS subject-scheme round-trip (infra smoke)', () => {
  it('a chat published to fleet.alpha.to.beta.chat is received by a beta-scoped subscriber', async () => {
    expect(serverStarts).toBe(true)
    const { connect } = await import('@nats-io/transport-node')
