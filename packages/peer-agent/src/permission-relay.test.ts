...
const REQUEST_MSG_ID = 'msg_01HR0000000000000000000001'

function fakeClient() {
  const sent: OutboundMessage[] = []
  return {
    sent,
    client: { send: async (m: OutboundMessage) => { sent.push(m); return { id: REQUEST_MSG_ID } as never } },
  }
}
...
describe('makeOutboundPermissionHandler — routing gate', () => {
  ...
  it('a failed send REVOKES that target\'s authorization (actually-relayed invariant)', async () => {
    ...
    expect(tracker.isAuthorizedResponder('abcde', 'bob', REQUEST_MSG_ID)).toBe(false)
  })

  it('SEC-M1: confirms the relay target against the exact outbound envelope id', async () => {
    ...
    expect(tracker.isAuthorizedResponder('abcde', 'bob', REQUEST_MSG_ID)).toBe(true)
    expect(tracker.isAuthorizedResponder('abcde', 'bob', 'msg_01HR0000000000000000000099')).toBe(false)
    expect(tracker.isAuthorizedResponder('abcde', 'mallory', REQUEST_MSG_ID)).toBe(false)
  })
})
